...
/* ================= BUSCA SEQUENCIAL ================= */
static int buscaSequencial(int m, int n, int valor, int *linhaEncontrada, int *colunaEncontrada) {
    int comparacoes = 0;
    int encontrado = 0;
    //'encontrado' no laço externo permitirá que a busca seja interrompida
    // assim que o valor for encontrado, evitando comparações desnecessárias. 
    for (int i = 0; i < m && !encontrado; i++) {
        for (int j = 0; j < n; j++) {
            comparacoes++; //conta toda comparação feita encontrada ou não
            if (matriz[i][j] == valor) {
                *linhaEncontrada = i;
                *colunaEncontrada = j;
                encontrado = 1;
...
static void preencherSequencial() {
  definirDimensoes();
  int valor = 0;
  for (int i = 0; i < Linhas; i++)
    for (int j = 0; j < Colunas; j++) {
...
/* ================= TESTE AUTOMATICO ================= */
static void testarTamanho(int m, int n) {
  //preenche a matriz com valores sequenciais (0, 1, 2,...)
  // para facilitar a previsibilidade dos resultados dos testes
  for (int i = 0; i < m; i++)
    for (int j = 0; j < n; j++)
      matriz[i][j] = i * n + j;
  ...
  //busca o penúltimo elemento da matriz, 
  //para diferenciar o resultado da "busca no final" do resultado da "busca inexistente"
  int valorFinal      = matriz[(totalElementos - 2) / n][(totalElementos - 2) % n]; 
  
  //soma um valor fora do intervalo gerado, garantindo que ele não exista na matriz
